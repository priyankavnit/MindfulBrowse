...
import { InsightsResponse, StoredEvent } from '@mindful-browse/shared';
import { queryEvents } from '../services/dynamodb';
...
export async function getInsights(
  ...
  try {
    ...
    const events = await queryEvents(userId, startTimestamp, now);

    logger.info('Retrieved events for insights', {
      userId,
    ...
    logger.error('Error retrieving insights', { error, userId });
...
function calculateInsights(events: StoredEvent[]): InsightsResponse {
  if (events.length === 0) {
    return {
  ...
  const total_time_seconds = events.reduce(
  ...
  for (const event of events) {
    if (event.sentiment in sentimentCounts) {
      sentimentCounts[event.sentiment as keyof typeof sentimentCounts]++;
    }
  }
  ...
  for (const event of events) {
    if (event.category in categoryCounts) {
      categoryCounts[event.category as keyof typeof categoryCounts]++;
    }
  }
...
function countDoomscrollSessions(events: StoredEvent[]): number {
