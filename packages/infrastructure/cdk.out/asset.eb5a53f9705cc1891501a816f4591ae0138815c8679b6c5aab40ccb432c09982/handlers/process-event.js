...
async function processEvent(event, userId) {
    try {
        ...
        if (browsingEvent.duration_seconds < SHORT_DURATION_THRESHOLD) {
            ...
            const classification = await (0, bedrock_1.classifyContent)(browsingEvent.domain, browsingEvent.title, browsingEvent.url);
        ...
        if (isDoomscroll) {
            ...
            if (isEligible) {
                ...
                const sessionDuration = Math.round((recentEvents.reduce((sum, e) => sum + e.duration_seconds, 0) +
